...
import { MessageSquare, X, Send, Phone, User, MessageCircle, SendHorizontal, Loader2, Sparkles, ChevronLeft, Instagram, Facebook } from 'lucide-react';
...
export const ContactChatbot: React.FC = () => {
  ...
  const contactOptions = [
    ...
    { 
      ...
      action: () => window.open('https://instagram.com/mindfulness_studio', '_blank')
    },
    { 
      name: 'Facebook', 
      color: 'bg-[#F0F2FF]', 
      ...
      action: () => window.open('https://facebook.com/mindfulness_studio', '_blank')
    },
    { 
      name: 'Appelez-nous', 
      color: 'bg-[#F8F9FA]', 
    ...
    { 
      ...
      action: () => {
        ...
        if (messages.length === 0) {
          setMessages([{ role: 'assistant', text: "Bonjour ! Je suis l'assistant de Fabienne. Comment puis-je vous éclairer aujourd'hui sur nos formations ou nos soins ?" }]);
        }
      }
    },
  ];
            ...
            <div className="grid grid-cols-1 gap-3">
              {contactOptions.map((opt, i) => (
                ...
                >
                  ...
                  <span className={cn("text-base font-bold tracking-tight", opt.textColor)}>
                    {opt.name}
                  </span>
                </button>
              ))}
          ...
          <div className="h-[520px] flex flex-col bg-white">
            <div className="p-4 bg-indigo-600 text-white flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-[10px] opacity-80 uppercase tracking-widest font-black">Agent IA • Actif</p>
              </div>
              <button onClick={() => setIsOpen(false)} className="hover:bg-white/20 p-1.5 rounded-lg">
                <X size={20} />
              </button>
            </div>
